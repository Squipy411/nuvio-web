...
const fixture = http.createServer(async (req, res) => {
  ...
  const name = path.replace(/^headers\//, "");
  ...
  res.writeHead(range ? 206 : 200, { "content-type": name.endsWith("m3u8") ? "application/vnd.apple.mpegurl" : name.endsWith("mp4") ? "video/mp4" : "application/octet-stream", "content-length": end - start + 1, "accept-ranges": "bytes", ...(range ? { "content-range": `bytes ${start}-${end}/${info.size}` } : {}) });
