...
export function ffmpegArguments(input: { url: string; directory: string; mode: PlaybackMode; position: number; audioIndex: number; probe: MediaProbe; resolution: PlaybackPreferences["resolution"] }): string[] {
  ...
  const args = ["-hide_banner", "-nostdin", "-loglevel", "error", "-y", "-protocol_whitelist", "http,tcp", "-format_whitelist", INPUT_FORMATS, "-rw_timeout", "15000000", "-analyzeduration", "5000000", "-probesize", "5000000", "-readrate", "1", "-readrate_initial_burst", "12"];
  if (input.position > 0) args.push("-ss", input.position.toFixed(3));
  args.push("-i", input.url, "-map", "0:v:0?");
  ...
  args.push("-max_muxing_queue_size", "1024", "-avoid_negative_ts", "make_zero", "-f", "hls", "-hls_segment_type", "fmp4", "-hls_time", "2", "-hls_list_size", "12", "-hls_delete_threshold", "2", "-hls_flags", "delete_segments+independent_segments+temp_file", "-hls_fmp4_init_filename", "init.mp4", "-hls_segment_filename", join(input.directory, "segment-%06d.m4s"), "-progress", "pipe:1", join(input.directory, "index.m3u8"));
  return args;
}
